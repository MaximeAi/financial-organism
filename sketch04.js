// Financial Organism v0.8
// 手機選單修正版 + Asset_History CSV 讀取版
// 重點：UI 按鈕不再被 Canvas touch 事件吃掉
...
function createPanelUI() {
  ...
  toggleBtn.style("touch-action", "manipulation");
  toggleBtn.mousePressed(togglePanel);

  toggleBtn.elt.addEventListener(
    "touchstart",
    function (e) {
      e.preventDefault();
      e.stopPropagation();
  ...
  panel.style("z-index", "9998");
  panel.style("position", "fixed");
  panel.style("touch-action", "auto");

  panel.elt.addEventListener(
    "touchstart",
    function (e) {
      e.stopPropagation();
    },
  ...
  addMobileButtonEvent(generateBtn, updateFromInputs);
...
function isTouchOnUI() {
  if (touches.length === 0) return false;

  let x = touches[0].x;
  let y = touches[0].y;

  if (x >= 20 && x <= 74 && y >= 20 && y <= 74) {
    return true;
  }

  if (panelVisible) {
    let panelWidth = width < 600 ? width - 40 : 360;
    let panelHeight = height - 120;

    if (x >= 20 && x <= 20 + panelWidth && y >= 88 && y <= 88 + panelHeight) {
      return true;
    }
  }
